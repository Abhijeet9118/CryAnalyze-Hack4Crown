              ...
              {result && !isAnalyzing && (
                <div className="space-y-6 animate-fadeIn">
                  ...
                  <div className="glass-panel p-6 rounded-2xl border border-slate-800/80 shadow-xl">
                    ...
                    {/* Ranked Multi-Reason Probability Matrix */}
                    <div className="space-y-3 mb-6 bg-slate-950/60 p-4 rounded-xl border border-slate-800/80">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center space-x-1.5">
                          <Activity size={14} className="text-cyan-400" />
                          <span>Ranked Acoustic Probability Matrix</span>
                        </span>
                        <span className="text-[10px] text-slate-500 font-mono">Highest % = Primary Reason</span>
                      </div>

                      <div className="space-y-2.5">
                        {(result.ranked_probabilities || Object.entries(result.probabilities || {}).map(([k, v], i) => ({ label: k, percentage: Math.round(v * 100), is_primary: k === result.prediction, rank: i + 1 }))).map((item) => (
                          <div 
                            key={item.label} 
                            className={`p-2.5 rounded-lg border transition-all ${
                              item.is_primary 
                                ? 'bg-slate-900/90 border-cyan-500/60 shadow-md shadow-cyan-500/10' 
                            ...
                            <div className="flex items-center justify-between text-xs mb-1.5">
                              <div className="flex items-center space-x-2">
                                <span className={`w-5 h-5 rounded-full flex items-center justify-center font-mono text-[10px] font-bold ${
                                  item.is_primary ? 'bg-cyan-500 text-slate-950 font-black' : 'bg-slate-800 text-slate-400'
                                }`}>
                                  #{item.rank}
                                </span>
                                <span className={item.is_primary ? 'text-white font-bold' : 'text-slate-400'}>
                                  {item.label}
                                </span>
                                {item.is_primary && (
                                  <span className="text-[9px] font-bold tracking-wider uppercase px-1.5 py-0.5 rounded bg-cyan-950 text-cyan-300 border border-cyan-800/80">
                                    Primary Match
                                  </span>
                                )}
                              </div>
                              <span className={`font-mono font-bold ${item.is_primary ? 'text-cyan-400 text-sm' : 'text-slate-400'}`}>
                                {item.percentage}%
                              </span>
                            </div>
                            ...
                            <div className="w-full bg-slate-950 h-2 rounded-full overflow-hidden">
                              <div 
                                className={`h-full rounded-full transition-all duration-700 ${
                                  item.is_primary 
                                    ? (result.escalate ? 'bg-gradient-to-r from-rose-500 to-red-600' : 'bg-gradient-to-r from-cyan-400 via-blue-500 to-indigo-500') 
                                    : 'bg-slate-700/50'
                                }`}
